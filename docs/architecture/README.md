...
`cw_api3d` is a guest DLL inside cadwork 3D, not a standalone Qt application. The architecture is hexagonal: application and ports never include Qt or CwAPI3D. Composition is the only module that names both sides.
