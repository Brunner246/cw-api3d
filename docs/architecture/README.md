...
`cw_api3d_charts` is a guest DLL inside cadwork 3D, not a standalone Qt application. Logging and plugin-path bootstrap load first, then element snapshot, aggregation, and a dockable Qt Graphs panel. Application and ports never include Qt or CwAPI3D. Composition is the only module that names both sides.

Configure the repo root (`cmake --preset local-relwithdebinfo`); the build produces one DLL, `cw_api3d_charts.dll`, deployed to `API.x64/cw_api3d_charts/`.
