...
2. **Python End-to-End (E2E) Test Harness** (pytest + uv) — Live integration tests launching cadwork 3D headlessly, running the embedded driver script, executing `cw_api3d.dll`, and verifying execution status.
...
## 3. Running C++ Tests (GoogleTest / CTest)
...
## 4. Running Python E2E Tests (pytest)
...
The CMake build automatically copies `cw_api3d.dll` and runtime dependencies (`spdlog.dll`, `fmt.dll`) into the active cadwork userprofile directory (`<CADWORK_USP>/3d/API.x64/cw_api3d/`):
```bash
cmake --build out/build/local-relwithdebinfo
```
...
## 5. E2E Environment Configuration & Overrides
...
| `CW_API3D_PLUGIN_DLL` | Path to the deployed `cw_api3d.dll` | `<CADWORK_USP>/3d/API.x64/cw_api3d/cw_api3d.dll` |

---

## 6. E2E Harness Architecture & Flow
...
sequenceDiagram
    ...
    participant Plugin as cw_api3d.dll (C++)
