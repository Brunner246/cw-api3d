...
.DESCRIPTION
  ...
  -ProfileRoot, recorded in the worktree's .cw-userprofile marker). Without that, every worktree
  post-build-copies over the same <userprofil>\3d\API.x64\cw_api3d_charts folder -- so two
  worktrees overwrite each other's cw_api3d_charts.dll, the e2e harness tests whichever built
  last, and a build fails outright whenever a running cadwork holds the DLL open.
